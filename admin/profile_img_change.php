	...
	if(!empty($photo)){
		...
		if($_FILES['photo']['size']<2000000){
			...
			if( in_array(strtolower($photo_ext),$accepted_extension_list) ){
				...
				$_SESSION['profile_photo_extension'] = "Attach a jpg,png or jpeg photo";
				header('location: profile.php');
				ob_end_flush();
			}
			...
			$_SESSION['profile_photo_size'] = "Attach a photo under 1MB";
			header('location: profile.php');
			ob_end_flush();
		}
		...
		// if empty
		$_SESSION['profile_photo_emty'] = "Attach a Photo";
		header('location: profile.php');
		ob_end_flush();
	}
